...
@Component({
  selector: 'app-test-detail-edit',
  templateUrl: './test-detail-edit.component.html',
  styleUrls: ['./test-detail-edit.component.scss']
})
export class TestDetailEditComponent implements OnInit {
  [x: string]: any;
  input: any;
  ...
  constructor(private changeDetection: ChangeDetectorRef, @Inject(SESSION_STORAGE) private storage: WebStorageService, private _PatientService: PatientService
    ...
    private _SoloAdminMasterComponent: SoloAdminMasterComponent,private modalService: ModalService) { }
  dochdrid;
  TestData: any = [];
  TestData2: any = [];
  ...
  getTESTData() {
    ...
    this._PatientService.GetPatientTestDetail(this.storage.get('ID')).subscribe(
      (res: any) => {
        // console.log(res[19].PREDEFVALModel);
        // this.TestData2=res[19].PREDEFVALModel
        this.TestData = res;
 ...
 // localStorage.setValue("input1").value=20;
 // console.log( localStorage.setValue("input1",20));
  ...
  // });
        ...
        //  localStorage.getElementById("input1").value = "20";

        //  console.log( localStorage.getElementById("input1").value);
        //  console.log ("helloby");
    ...
    //  if( Value<List.TESTDET_MaxValue && Value>List.TESTDET_MinValue)
    // {
  ...
  // isChangeLimitAccessToggle_new(value1)
  // {
  //   this.input=value1;
  ...
  Back(Patient_Id) {
    ...
    this.router.navigateByUrl('/Admins/PatientTestEntery');
