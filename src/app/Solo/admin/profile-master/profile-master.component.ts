...
export class ProfileMasterComponent implements OnInit {
  ...
  constructor(private _PatientService: PatientService, private modalService: ModalService, private _SoloAdminMasterComponent: SoloAdminMasterComponent, private _TestService: TestService,
    ...
    , private router: Router, private alertService: AlertService,) {
      ...
      localStorage.setItem('PageTitle', "Profile Test Add"); }
  f_TestName;
  f_TestType = 1;
  f_ReportHeading;
  f_Rate;
  ...
  getAllData(NO) { 
    ...
    let model = {
      PageNo: NO,
      PageSize: this.TESTMST_CurrentId,
      Keyword: null,
  ...
  insert(modal) {
    this._SoloAdminMasterComponent.setLoading(true);
    this._TestService.InsertProfileTest(modal).subscribe(
      (res: any) => {
        if (res != 0) {
          this._SoloAdminMasterComponent.setLoading(false);
          this.storage.remove('ID');
          localStorage.setItem("AlertType","success");
          localStorage.setItem("AlertMessage","Profile Test Created Succesfully"); 
  ...
  update(modal) {
    this._SoloAdminMasterComponent.setLoading(true);
    this._TestService.UpdateProfileTest(modal).subscribe(
      (res: any) => {
        this._SoloAdminMasterComponent.setLoading(false);
        if (res != 0) {
          this._SoloAdminMasterComponent.setLoading(false);
          this.storage.remove('ID');
          localStorage.setItem("AlertType","success");
          localStorage.setItem("AlertMessage","Profile Test Updated Succesfully"); 
          this.alertService.error("Test Created Successfully..!");
          this.router.navigateByUrl('Admin/PathologyTestList');
        }
      },
