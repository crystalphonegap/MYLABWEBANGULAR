...
@Component({
  ...
  templateUrl: './collection-center-list.component.html',
  styleUrls: ['./collection-center-list.component.scss']
})
export class CollectionCenterListComponent implements OnInit {

  DataList: any=[];
  constructor(private router: Router, private _CollectionCenterService: CollectionCenterService,private _PatientService:PatientService,
  ...
  getAllDataCount() {
    ...
    this._CollectionCenterService.GetAllCollectionCenterDetailsCount(model).subscribe((data: any) => {
  ...
  Edit(Value){
    ...
    this.router.navigateByUrl('/Admins/CollectionCenterAdd');
  }
  ...
  Delete(Value){
    ...
    this.closeModal(Value);
     this._PatientService.GlobalDelete("CollectionCenter",Value).subscribe((res: any) => { 
      if(res==-7){
        this.alertService.info("You can not delete this Collection Center , This Collection Center is in use ");
      }else{
        this.alertService.success("Collection Center Deleted Successfully");
        this.changeDetection.detectChanges();
         this.getAllData(1);
      } 
      this._LayoutComponent.setLoading(false);
