...
export class ReasonMasterComponent implements OnInit {
  ...
  getAllData(pageNo) {
    ...
    this._TestService.GetReSampleReasonListSearch(model).subscribe((res: any) => {  
  ...
  getAllDataCount() {
    ...
    this._TestService.GetReSampleReasonSearchCount(model).subscribe((data: any) => {
  ...
  Edit(Value){
    ...
    this.router.navigateByUrl('/Admins/ReasonAdd');
  }
  ...
  Delete(Value){
    ...
    this.closeModal(Value);
     this._PatientService.GlobalDelete("Reason Master",Value).subscribe((res: any) => {
      if(res==-7){
        this.alertService.info("You can not delete this Reason Master , This Reason Master is in use ");
        this.changeDetection.detectChanges();
      }else{
        this.alertService.success("Reason Master Deleted Successfully");
        this.changeDetection.detectChanges();
         this.getAllData(1);
      }  
      this._LayoutComponent.setLoading(false);
