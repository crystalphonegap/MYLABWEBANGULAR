...
export class NarrationListComponent implements OnInit {
  ...
  getAllData(pageNo) {
    ...
    this._TestService.GetNarrationSearch(model).subscribe((res: any) => {  
  ...
  getAllDataCount() {
    ...
    this._TestService.GetNarrationSearchCount(model).subscribe((data: any) => {
  ...
  Edit(Value){
    ...
    this.router.navigateByUrl('/Admins/NarrationAdd');
  }
  ...
  Delete(Value){
    ...
    this.closeModal(Value);
     this._PatientService.GlobalDelete("Narration",Value).subscribe((res: any) => {
      if(res==-7){
        this.alertService.info("You can not delete this Narration , This Narration is in use ");
        this.changeDetection.detectChanges();
      }else{
        this.alertService.success("Narration Deleted Successfully");
        this.changeDetection.detectChanges();
         this.getAllData(1);
      }  
      this._LayoutComponent.setLoading(false);
